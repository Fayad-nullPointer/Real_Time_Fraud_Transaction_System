...
class KafkaStreamState:
    def __init__(self):
        ...
        self.speed = 0.8
        self.max_tx = 100
        self.dataset = "test_transactions_first_200.csv"
...
class StartStreamRequest(BaseModel):
    speed: float = 0.8
    max_tx: int = 100
    dataset: str = "test_transactions_first_200.csv"


async def _run_gui_stream_loop(speed: float, max_tx: int, dataset: str = "test_transactions_first_200.csv"):
    ...
    _kafka_gui_stream.dataset = dataset
    ...
    if csv_path and csv_path.exists():
        try:
            ...
            print(f"[gui-stream] Successfully loaded {len(stream_rows)} transactions from {csv_path.name}")
...
async def start_gui_kafka_stream(body: StartStreamRequest, _: dict = Depends(get_current_admin)):
    ...
    _kafka_gui_stream.task = asyncio.create_task(_run_gui_stream_loop(body.speed, body.max_tx, body.dataset))
    return {"status": "STARTED", "message": f"Started Kafka stream simulation ({body.max_tx} txns @ {body.speed}s from {body.dataset})."}
...
async def get_gui_kafka_stream_status(_: dict = Depends(get_current_admin)):
    return {
        ...
        "dataset": _kafka_gui_stream.dataset,
    }